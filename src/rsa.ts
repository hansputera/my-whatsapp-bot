import {
    createPrivateKey, createPublicKey, KeyObject, privateDecrypt, publicEncrypt} from 'node:crypto';
  ...
  constructor() {
    if (existsSync(resolve(__dirname, '..', 'keys')) && process.env.KEY) {
      let privateKeyContent = process.env.PRIVATE_KEY ||
            readFileSync(resolve(__dirname, '..', 'keys', 'private.key.pem'), {
              'encoding': 'utf8',
            });
      if (process.env.PRIVATE_KEY) {
        privateKeyContent = inflateSync(Buffer.from(privateKeyContent)).toString('hex');
        privateKeyContent = inflateSync(Buffer.from(privateKeyContent, 'hex')).toString('utf8');
      }
      const publicKeyContent = readFileSync(resolve(
