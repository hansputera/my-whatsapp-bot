...
const ImageToSticker: CommandFunc = async (
    ctx: Context,
) => {
  try {
    let image: Image;
    ...
    if (reply) {
      if (!reply.image) {
        await ctx.reply('Make sure you have reply a message contains image.');
        return;
      }
      ...
      if (!ctx.image) {
        await ctx.reply('Please send an image using caption or reply' +
