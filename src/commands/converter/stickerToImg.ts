...
const stickerToImg: CommandFunc = async (
    ctx: Context,
) => {
  ...
  if (!replied) {
    ...
    if (!sticker) {
      await ctx.reply('You have reply wrong message, ' +
      ...
      if (isGIF && !sticker.animated) {
        await ctx.reply('Are you trying to convert an image to ' +
                'a GIF?');
        return;
      }
      try {
...
export default Util.makeCommandConfig({
  ...
  'alias': ['stoimg', 'toimg', 'sticker-to-img', 'stickertoimage', 'stoimage'],
  'target': stickerToImg,
});
