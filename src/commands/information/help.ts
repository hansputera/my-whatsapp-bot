...
const HelpCommand: CommandFunc = async (ctx: Context) => {
	const command = ctx.args.shift();
	if (!command) {
		await ctx.reply(
			'There are *' +
				ctx.client.modules.commands.size +
				' commands* is available!\n\n' +
				[...ctx.client.modules.mods.values()]
					.map(
						(m) =>
							'*- ' +
							m.name +
							'*\n' +
							m.commands?.map((c) => c.name).join(', '),
					)
					.join('\n'),
		);
	} else {
		const cmd =
			ctx.client.modules.commands.get(command) ||
			[...ctx.client.modules.commands.values()].find((c) =>
				c.alias?.includes(command.toLowerCase()),
			);
		if (!cmd) {
			const probabilityCmds = [...ctx.client.modules.commands.values()]
				.filter(
					(c) =>
						c.name.startsWith(command.toLowerCase()) ||
						c.alias?.find((a) =>
							a.startsWith(command.toLowerCase()),
						),
				)
				.map((c) => c.name);
