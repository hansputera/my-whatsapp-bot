import got from 'got';
import { MediaType, getMediaKeys } from '@adiwajshing/baileys';
...
export class Util {
	...
	static async postToHastebin(text: string): Promise<string> {
		...
		return (
			'https://www.toptal.com/developers/hastebin/raw/' +
	...
	static mockText(text: string): string {
		const lets: string[] = text.split('');

		for (let i = 0; i < lets.length; i += Math.floor(Math.random() * 4)) {
			lets[i] = lets[i].toUpperCase();
		}
