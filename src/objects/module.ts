...
export class Modules {
	...
	loads(): void {
		if (!existsSync(this.commandsPath)) {
			throw new TypeError('Commands Path location could not be found');
		}
		this.free();
		for (const modFolder of readdirSync(this.commandsPath)) {
			if (
				!existsSync(
					path.resolve(this.commandsPath, modFolder, '__config.js'),
				)
			) {
				throw new Error(
					modFolder + " couldn't found the module configuration!",
				);
			}
			...
			modConfig.commands = [];
			for (const cmdFile of readdirSync(
				path.resolve(this.commandsPath, modFolder),
			).filter((fl) => fl.endsWith('.js') && !fl.startsWith('__'))) {
				this.logger.info('Load ' + cmdFile + ' in ' + modFolder);
				const defaultCmd: CommandInfo = require(path.resolve(
					this.commandsPath,
					modFolder,
					cmdFile,
				)).default;
				...
				this.logger.info(
					cmdFile + ' loaded correctly with name: ' + defaultCmd.name,
				);
			}
			this.mods.set(modConfig.name, modConfig);
			this.logger.info(
				modFolder +
					' loaded correctly with ' +
					modConfig.commands.length +
					' commands',
			);
		}
	}
	...
	loadEvents(): void {
		...
		for (const eventFile of readdirSync(this.eventsPath).filter((fl) =>
			fl.endsWith('.js'),
		)) {
			this.logger.info('Load event file: ' + eventFile);
			const eventFl: EventInfo = require(path.resolve(
				this.eventsPath,
				eventFile,
			)).default;
			...
			this.client.baileys.ev.on(eventFl.name, (arg) =>
				eventFl.target(this.client, arg),
			);
