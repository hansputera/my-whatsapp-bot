...
export class Context {
	...
	public getGroup(): GroupContext | undefined {
		return this.client.groupsCache.get(this.currentJid() + '@g.us');
	}
	...
	public getArgs(withPrefix: boolean = false): string[] {
		...
		if (
			...
			text +=
				' ' + extendedMessage.contextInfo.quotedMessage.conversation;
		}
	...
	public get command(): CommandInfo | undefined {
		...
		return (
			this.client.modules.commands.get(cmd.toLowerCase()) ||
			[...this.client.modules.commands.values()].find((c) =>
	...
	public getPrefix(): string {
		if (!this.text) return '';
		const p = prefixes.find((p) => this.text.startsWith(p.toLowerCase()));
	...
	public getReply(): ContextInfo | undefined {
		if (
			this.msg.message?.extendedTextMessage &&
	...
	public get authorNumber(): string | undefined {
		return this.msg.key.participant
			? this.msg.key.participant.replace(/\@.+/gi, '')
			: this.isFromMe
			? this.client.baileys.user.id.replace(/\@.+/gi, '').split(':')[0]
			: this.currentJid();
	}
	...
	public get participant(): GroupParticipant | undefined {
		if (!this.isGroup) return undefined;
			...
			this.isGroup &&
			!this.client.groupsCache.has(this.msg.key.remoteJid as string)
		) {
			return undefined;
		}

		return this.client.groupsCache
			.get(this.msg.key.remoteJid as string)
	...
	public currentJid(): string {
		return this.msg.key.remoteJid
			? this.msg.key.remoteJid.replace(/\@.+/gi, '')
			: '';
	}
	...
	public get isGroup(): boolean {
		return !((this.authorNumber as string) === this.currentJid());
	}
	...
	public get sticker(): Sticker | undefined {
		if (this.msg.message?.stickerMessage) {
			return new Sticker(this.msg.message.stickerMessage);
	...
	public get image(): Image | undefined {
		if (this.msg.message?.imageMessage) {
			return new Image(this.msg.message.imageMessage);
		} else return undefined;
	}
	...
	public get video(): Video | undefined {
		if (this.msg.message?.videoMessage) {
			return new Video(this.msg.message.videoMessage);
		} else return undefined;
	}
	...
	public async syncGroup(): Promise<boolean> {
		...
		const groupMeta = await this.client.baileys
			.groupMetadata(this.msg.key.remoteJid as string)
