...
const groupParticipantUpdateHandle = async (
	...
	const group = client.groupsCache.get(arg.id);

	if (
		group &&
		arg.action === 'remove' &&
		arg.participants.includes(client.baileys.user.id.replace(':1', ''))
	) {
		client.logger.info('Got kicked from: ' + group.name);
		client.groupsCache.delete(arg.id);

		if (client.youtubeStreams.has(arg.id)) {
			client.youtubeStreams.get(arg.id)?.destroy(new Error('leaved'));
		}

		const gCollectors = [...client.collectors.keys()].filter((key) =>
			key.includes(arg.id.replace(/\@.+/gi, '')),
		);
